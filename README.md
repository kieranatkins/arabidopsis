...
```
Note: There are ``beta``, ``v3`` and ``v4`` branches of this image that contains model weights trained on more diverse samples. To use this beta branch, the name must be provided every time you run a docker command. (i.e. ``docker pull kieranatkins/silique-detector:beta`` and ``docker run --shm-size=512m kieranatkins/silique-detector:beta test``)
...
**Step 3 (v4 only):**
The docker image `v4` contains the extra function ``phenotype`` that will take the outputs generated by the ``inference`` command and create a single ``.csv`` file with the generated phenotype data for each detected silique. To generate the phenotype data after running inference, run the command:
```
docker run -v /path/to/my_data:/data --shm-size=512m kieranatkins/silique-detector:v4 phenotype "/data/out/preds"
```
